 ...
 *  of each ith integer in this array is defined as the product of the following:<ul>
...
such that A[j]>A[i] (j>i). If multiple A[j]s are present in multiple positions,
...
public class MAXSPPROD {
	...
	public static int maxSpecialProduct(ArrayList<Integer> A) {
		...
		for (int i = 1; i < A.size(); i++) {
			while (!stack.isEmpty() && A.get((int) stack.peek()) <= A.get(i))
				stack.pop();
			if (stack.isEmpty())
				left_max.add(0);
			else
		...
		System.out.println("left max" + left_max);
		...
		for (int i = A.size()-2; i >= 0; i--) {
			...
			if (stack.isEmpty())
				...
				right_max.add(0, (int) stack.peek() );
			stack.push(i);
		}

		System.out.println("right max" + right_max);
		
		long max_sp=0;
		for (int i = 0; i <right_max.size(); i++) {
			max_sp = Long.max(max_sp, left_max.get(i) * right_max.get(i));
		}
		return (int)max_sp%1000000007;
	}
